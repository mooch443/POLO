...
!!! example
    ...
    === "Syntax"
        ...
        Where:
        - `TASK` (optional) is one of [detect, locate, segment, semantic, classify, pose, obb]
...
- `TASK` (optional) is one of `[detect, locate, segment, semantic, classify, pose, obb]`. If not explicitly passed, YOLO will attempt to infer the `TASK` from the model type.
- `MODE` (required) is one of `[train, val, predict, export, track, benchmark]`
