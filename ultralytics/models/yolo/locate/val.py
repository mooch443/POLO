...
class LocalizationValidator(BaseValidator):
    ...
    def preprocess(self, batch):
        ...
        if self.args.save_hybrid:
            ...
            nb = len(batch["img"])
            locations = batch["locations"] * torch.tensor((width, height), device=self.device)
            self.lb = (
                [
    ...
    def _prepare_pred(self, pred, pbatch):
        """Prepares a batch of images and annotations for validation."""
        predn = pred.clone()
        ops.scale_locations(pbatch["imgsz"], predn[:, :2], pbatch["ori_shape"], ratio_pad=pbatch["ratio_pad"])  # pred
        return predn
    ...
    def _process_batch(self, detections, gt_locations, gt_cls):
        ...
        radii = self.radii if self.radii is not None else self.data["radii"]
        gt_radii = ops.generate_radii_t(radii=radii, cls=gt_cls).view(-1, 1)
