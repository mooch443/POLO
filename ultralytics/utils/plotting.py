...
class Annotator:
    ...
    def loc_label(
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                w, h = self.font.getsize(label)  # text width, height
                outside = bbox_tl[1] - h >= 0
                self.draw.rectangle(
                    (
                        bbox_tl[0],
                        bbox_tl[1] - h if outside else bbox_tl[1],
                        bbox_tl[0] + w + 1,
                        bbox_tl[1] + 1 if outside else bbox_tl[1] + h + 1,
                    ),
                    fill=color,
                )
                self.draw.text(
                    (bbox_tl[0], bbox_tl[1] - h if outside else bbox_tl[1]),
                    label,
                    fill=txt_color,
                    font=self.font,
                )
...
def plot_labels_loc(locations, cls, names=(), save_dir=Path(""), on_plot=None):
    """Plot class histograms for localization labels."""
...
def plot_images(
    ...
    batch_idx: torch.Tensor | np.ndarray | None = None,
    ...
    Args:
        labels (dict[str, Any] | None): Optional dictionary containing detection data with keys like 'cls', 'bboxes',
            'locations', 'radii', 'conf', 'masks', 'semantic_mask', 'keypoints', 'batch_idx', 'img'. If provided, it
            overrides individual args.
    ...
    """
    semantic_masks = np.zeros(0, dtype=np.int64)
    ...
    if batch_idx is None:
        batch_idx = np.zeros(cls.shape, dtype=np.int64)
    ...
    for i in range(bs):
        ...
        if len(cls) > 0:
            ...
            if len(bboxes):
                boxes = bboxes[idx]
                has_boxes = len(boxes) and not (len(locations) and np.all(boxes == 0))
                if has_boxes:
