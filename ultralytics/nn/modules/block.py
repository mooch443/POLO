...
class LocInf(nn.Module):
    ...
    scales. Adjusted from DFL loss. The intuition behind this is a bit unclear
    """
    ...
    def __init__(self, c1=16):
        ...
        super().__init__()
        self.conv = nn.Conv2d(c1, 1, 1, bias=False).requires_grad_(False)
        x = torch.linspace(start=0,end=1,steps=c1,dtype=torch.float, device=self.conv.device)
    ...
    def forward(self, x):
        ...
        b, c, a = x.shape  # batch, channels (reg_max*4), anchors
        '''
